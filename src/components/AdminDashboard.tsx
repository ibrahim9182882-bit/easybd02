    ...
    try {
      ...
      if (nextStatus === "rejected" && !record.refundProcessed) {
        ...
        if (userSnap.exists()) {
          ...
          const newBal = currentBal + record.amount;
          const newTot = currentTot + record.amount;
      ...
      if ((nextStatus === "paid" || nextStatus === "pending") && record.status === "rejected" && record.refundProcessed) {
        ...
        if (userSnap.exists()) {
          ...
          const currentBal = uData.balance || 0;
          const currentTot = uData.totalEarned || 0;
          const newBal = Math.max(0, currentBal - record.amount);
          const newTot = Math.max(0, currentTot - record.amount);
