...
import { formatRateFormula } from "../utils/conversion";
...
export default function AdminDashboard({ onClose, onShowToast, currentSettings }: AdminDashboardProps) {
  ...
  // Check if current authenticated user is already admin & listen to real-time updates
  useEffect(() => {
    let unsubWithdrawals: (() => void) | null = null;

    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      if (user && user.email === "ibrahim9182882@gmail.com") {
        setIsAdminAuth(true);
        fetchData();

        // Subscribe to real-time public withdrawals updates
        if (!unsubWithdrawals) {
          unsubWithdrawals = onSnapshot(
            collection(db, `${artifactRoot}/public/data/withdrawals`),
            (snap) => {
            ...
            (err) => console.warn("Realtime withdrawals listener notice:", err)
          );
        }
      }
    });
  ...
  const fetchData = async () => {
    ...
    // 1. Fetch Users using multi-strategy query (Public Collection -> Collection Group -> Referral Mapping)
    try {
      const userMap = new Map<string, UserProfile>();

      // Strategy A: Public Users Collection
      try {
        const publicUsersSnap = await getDocs(collection(db, `${artifactRoot}/public/data/users`));
        publicUsersSnap.forEach((docSnap) => {
          const data = docSnap.data();
          const uid = docSnap.id || data.uid;
          if (uid) {
            userMap.set(uid, { uid, ...data } as UserProfile);
          }
        });
      } catch (e) {
        console.warn("Public users fetch notice:", e);
      }

      // Strategy B: Collection Group Query for Profile Subcollections
      try {
        const querySnapshot = await getDocs(query(collectionGroup(db, "profile")));
        querySnapshot.forEach((docSnap) => {
          const data = docSnap.data();
          const uid = docSnap.ref.parent.parent?.id || data.uid;
          if (uid) {
            userMap.set(uid, {
              ...userMap.get(uid),
              ...data,
        ...
        console.warn("Collection group profile notice:", cgErr);
      }

      // Strategy C: Referral Codes directory -> fetch individual user profiles if any missing
      try {
        const refCodesSnap = await getDocs(collection(db, `${artifactRoot}/public/data/referralCodes`));
        ...
        await Promise.all(
          missingUids.map(async (uId) => {
            try {
              const uSnap = await getDoc(doc(db, `${artifactRoot}/users/${uId}/profile`, "main"));
              if (uSnap.exists()) {
                userMap.set(uId, { uid: uId, ...uSnap.data() } as UserProfile);
              }
            } catch (pErr) {
              console.warn(`Profile fetch for ${uId} notice:`, pErr);
            }
          })
        );
      } catch (refErr) {
        console.warn("Referral codes directory notice:", refErr);
      }
    ...
    // 2. Fetch ALL withdrawals from public pool + collection group
    try {
      const withdrawMap = new Map<string, WithdrawalRecord>();

      // Primary: public withdrawals pool
      const wSnap = await getDocs(collection(db, `${artifactRoot}/public/data/withdrawals`));
      wSnap.forEach((docSnap) => {
        withdrawMap.set(docSnap.id, {
          id: docSnap.id,
          ...docSnap.data()
        } as WithdrawalRecord);
      });

      // Secondary: subcollection withdrawals search
      try {
        const cgSnap = await getDocs(query(collectionGroup(db, "withdrawals")));
        cgSnap.forEach((docSnap) => {
          if (!withdrawMap.has(docSnap.id)) {
            withdrawMap.set(docSnap.id, {
              id: docSnap.id,
              ...docSnap.data()
            } as WithdrawalRecord);
        ...
        console.warn("Collection group withdrawals notice:", cgErr);
      }

      const withdrawList = Array.from(withdrawMap.values());
  ...
  const handleToggleUserStatus = async (user: UserProfile) => {
    ...
    try {
      ...
      await setDoc(doc(db, `${artifactRoot}/public/data/users`, user.uid), { disabled: updatedStatus }, { merge: true });
  ...
  const handleDeleteUserDoc = async (user: UserProfile) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, `${artifactRoot}/public/data/users`, user.uid));
      if (user.referralCode) {
        try {
          await deleteDoc(doc(db, `${artifactRoot}/public/data/referralCodes`, user.referralCode));
        } catch (e) {
          console.warn("Referral code delete notice:", e);
        }
      }
      onShowToast(`User account deleted: ${user.firstName || "Member"}`, "success");
  ...
  const handleAdjustBalance = async (type: "add" | "deduct" | "gift") => {
    ...
    try {
      ...
      await setDoc(doc(db, `${artifactRoot}/public/data/users`, selectedUser.uid), {
        balance: newBalance,
        totalEarned: newTotal
      }, { merge: true });

      // Add Earning Event Log & Special Notification trigger
      let eventSource = "";
      if (type === "gift") {
        eventSource = `🎁 Special Admin Gift: ${giftReason.trim() || "Surprise Bonus!"}`;
      } else if (type === "add") {
        eventSource = `🎉 Admin Credit Bonus (+${amt} Coins)`;
      } else {
        eventSource = `⚠️ Admin Balance Adjustment (-${amt} Coins)`;
      }
      ...
      onShowToast(
        type === "gift"
          ? `🎁 Special gift of ${amt} coins sent to ${selectedUser.firstName || "User"}!`
          : `Balance adjusted for ${selectedUser.firstName || "User"} (${type === "add" ? "+" : "-"}${amt} coins)`,
        "success"
      );
  ...
  const handleUpdateWithdrawalStatus = async (
    ...
    nextStatus: "pending" | "paid" | "rejected"
  ) => {
    if (record.status === nextStatus) return;

    try {
      const recordRef = doc(db, `${artifactRoot}/public/data/withdrawals`, record.id);
      let updatedRefundProcessed = record.refundProcessed || false;

      // 1. Transition TO Rejected (and refund not yet processed)
      if (nextStatus === "rejected" && !record.refundProcessed) {
        try {
          ...
          if (userSnap.exists()) {
            ...
            const currentTot = Number(uData.totalEarned) || 0;
            const refundAmt = Number(record.amount) || 0;
            const newBal = currentBal + refundAmt;
            const newTot = currentTot + refundAmt;
            ...
            await setDoc(
              doc(db, `${artifactRoot}/public/data/users`, record.userId),
              { balance: newBal, totalEarned: newTot },
              { merge: true }
            );
          ...
          console.warn("User profile update error during withdrawal rejection:", e);
        }
        updatedRefundProcessed = true;
      }

      // 2. Transition FROM Rejected to Paid/Pending (reverse previous refund if it was processed)
      if ((nextStatus === "paid" || nextStatus === "pending") && record.status === "rejected" && record.refundProcessed) {
        try {
          const userRef = doc(db, `${artifactRoot}/users/${record.userId}/profile`, "main");
          const userSnap = await getDoc(userRef);
          ...
          if (userSnap.exists()) {
            ...
            const newTot = Math.max(0, currentTot - refundAmt);
            ...
            // Log reversal in earnings history
            const earningsRef = collection(db, `${artifactRoot}/users/${record.userId}/earnings`);
            await addDoc(earningsRef, {
              amount: -refundAmt,
              source: `Reversed Refund: Status updated to ${nextStatus.toUpperCase()} (${record.method})`,
              timestamp: serverTimestamp()
            });
          }
        } catch (e) {
          console.warn("User profile reversal error during status change:", e);
        }
        updatedRefundProcessed = false;
      }

      await updateDoc(recordRef, {
        status: nextStatus,
        refundProcessed: updatedRefundProcessed,
        updatedAt: serverTimestamp()
      });

      onShowToast(`Withdrawal marked as: ${nextStatus.toUpperCase()}`, "success");

      setWithdrawals((prev) =>
        prev.map((w) =>
          w.id === record.id
            ? { ...w, status: nextStatus, refundProcessed: updatedRefundProcessed }
            : w
        )
      );
    } catch (err: any) {
      console.error("Failed to update status:", err);
      onShowToast("Failed to update status: " + (err.message || "Unknown error"), "error");
    }
  };
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      await setDoc(settingsRef, {
        ...
        botUsername: settingsForm.botUsername || "easybdearnbot",
        adLink: settingsForm.adLink1 || settingsForm.adLink || "",
        adLink1: settingsForm.adLink1 || settingsForm.adLink || "",
        adLink2: settingsForm.adLink2 || settingsForm.adLink1 || settingsForm.adLink || "",
        adLink3: settingsForm.adLink3 || settingsForm.adLink2 || settingsForm.adLink1 || settingsForm.adLink || ""
      });
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right flex flex-col items-end gap-1">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              if (confirmingDeleteUid === user.uid) {
                                handleDeleteUserDoc(user);
                              } else {
                                setConfirmingDeleteUid(user.uid);
                              }
                            }}
                            className={`mt-1 py-1 px-2.5 rounded-lg text-[10px] font-bold flex items-center gap-1 transition active:scale-95 ${
                          ...
                          >
                            ...
                            <span>{confirmingDeleteUid === user.uid ? "Confirm Delete?" : "Delete"}</span>
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-2">
                      ...
                      {confirmingDeleteUid === selectedUser.uid ? (
                        <div className="flex gap-1.5 flex-1">
                          <button
                            type="button"
                            onClick={() => handleDeleteUserDoc(selectedUser)}
                            className="flex-1 py-3 px-2 rounded-xl text-xs font-black bg-rose-600 text-white flex items-center justify-center gap-1 hover:bg-rose-700 active:scale-95 transition shadow-lg shadow-rose-950/50"
                          >
                          ...
                          <button
                            ...
                            className="py-3 px-3 rounded-xl text-xs font-extrabold bg-slate-800 text-slate-300 hover:bg-slate-700 transition"
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => setConfirmingDeleteUid(selectedUser.uid)}
                          className="py-3 px-4 rounded-xl text-xs font-extrabold bg-rose-600/20 border border-rose-500/40 text-rose-400 flex items-center justify-center gap-1.5 hover:bg-rose-600 hover:text-white active:scale-95 transition"
                        >
                          <Trash2 className="w-4 h-4" />
                    ...
                    <div key={record.id} className="bg-slate-900 border border-slate-800/80 p-4 rounded-2xl space-y-3 text-left">
                      ...
                      <div className="pt-2 border-t border-slate-800/40 space-y-1.5">
                        <div className="flex items-center justify-between text-[10px] text-slate-500 font-extrabold uppercase">
                          <span>Change Status Action:</span>
                          <span className="text-slate-400 font-mono">Current: {record.status.toUpperCase()}</span>
                        </div>
                        <div className="grid grid-cols-3 gap-1.5">
                          <button
                            onClick={() => handleUpdateWithdrawalStatus(record, "pending")}
                            className={`py-1.5 px-2 rounded-xl text-[10px] font-extrabold flex items-center justify-center gap-1 transition active:scale-95 ${
        ...
        {!loadingData && activeTab === "settings" && (
          <div className="space-y-6 text-left">
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-4">
              {/* Coin Value */}
              <div>
                <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Coin Exchange Conversion Formula</label>
                <input
                  type="text"
                  value={settingsForm.coinValue}
                  onChange={(e) => setSettingsForm({ ...settingsForm, coinValue: e.target.value })}
                  placeholder="e.g. 500 Coins = 5 BDT, 200 Coins = 1 BDT, or 700 Coins = 5 BDT"
                ...
                <p className="text-[9px] text-slate-400 mt-1 font-mono">
                  Current Setting: <span className="text-emerald-400 font-bold">{formatRateFormula(settingsForm.coinValue)}</span>. (Examples: "500 Coins = 5 BDT", "200 Coins = 1 BDT", "700 Coins = 5 BDT")
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 10000, 500, 200"
                  className="w-full bg-[#080d1a] border border-slate-800 rounded-xl p-3 text-xs text-white focus:border-emerald-500 outline-none font-semibold"
                />
                <p className="text-[9px] text-slate-500 mt-1">Set any minimum threshold for user checkout requests (e.g., 200, 500, 10000 coins).</p>
              </div>
              ...
              <div>
                <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Telegram Referral Bot Username</label>
                <input
                  type="text"
                  value={settingsForm.botUsername || "easybdearnbot"}
                  onChange={(e) => setSettingsForm({ ...settingsForm, botUsername: e.target.value.replace("@", "").trim() })}
                  placeholder="e.g. easybdearnbot"
                  className="w-full bg-[#080d1a] border border-slate-800 rounded-xl p-3 text-xs text-white focus:border-emerald-500 outline-none font-semibold"
                />
                <p className="text-[10px] text-slate-500 mt-1">Share links will be generated as: https://t.me/yourbot?startapp=CODE</p>
              </div>

              {/* Sponsor Direct Link Advertisements Section */}
              <div className="pt-4 border-t border-slate-800/60 space-y-4">
                <h3 className="text-xs font-black uppercase text-slate-300 tracking-wider">Direct Sponsor Ad configuration (3 Steps)</h3>
                
                <div>
                  <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Sponsor Ad Page 1 URL</label>
                  <input
                    type="text"
                    value={settingsForm.adLink1 || settingsForm.adLink || ""}
                    onChange={(e) => setSettingsForm({ ...settingsForm, adLink1: e.target.value, adLink: e.target.value })}
                    placeholder="e.g. https://sponsor.com/page1"
                    className="w-full bg-[#080d1a] border border-slate-800 rounded-xl p-3 text-xs text-white focus:border-emerald-500 outline-none font-semibold"
                  />
                </div>

                <div>
                  <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Sponsor Ad Page 2 URL</label>
                  <input
                    type="text"
                    value={settingsForm.adLink2 || ""}
                    onChange={(e) => setSettingsForm({ ...settingsForm, adLink2: e.target.value })}
                    placeholder="e.g. https://sponsor.com/page2"
                    className="w-full bg-[#080d1a] border border-slate-800 rounded-xl p-3 text-xs text-white focus:border-emerald-500 outline-none font-semibold"
                  />
                </div>

                <div>
                  <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Sponsor Ad Page 3 URL</label>
                  <input
                    type="text"
                    value={settingsForm.adLink3 || ""}
                    onChange={(e) => setSettingsForm({ ...settingsForm, adLink3: e.target.value })}
                    placeholder="e.g. https://sponsor.com/page3"
                    className="w-full bg-[#080d1a] border border-slate-800 rounded-xl p-3 text-xs text-white focus:border-emerald-500 outline-none font-semibold"
                  />
                </div>

                <div>
                  <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Sponsor Ad Banner Image URL</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">Sponsor Completion Reward (Coins)</label>
                  <input
                    type="number"
