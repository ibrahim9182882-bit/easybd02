  ...
  const processReferral = async (code: string, currentUid: string) => {
    ...
    try {
      ...
      if (refCodeSnap.exists()) {
        ...
        // Fallback search across profile collection group
        const q = query(collectionGroup(db, "profile"), where("referralCode", "==", cleanedCode));
        const qSnap = await getDocs(q);
        if (!qSnap.empty) {
          referrerId = qSnap.docs[0].data().uid;
          if (referrerId) {
            await setDoc(doc(db, `${artifactRoot}/public/data/referralCodes`, cleanedCode), { userId: referrerId }, { merge: true });
          }
        }
      }
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (myData.referredBy || myData.referralCode === cleanedCode) {
          // Already referred or self
          return;
        }
        ...
        transaction.set(myEarningsRef, {
          amount: signupReward,
          source: `Signup Referral Bonus (${cleanedCode})`,
          timestamp: serverTimestamp(),
        });
      ...
      if (updatedMyData) {
        ...
        showToast(`Applied Referral Code! +${signupReward} Coins Bonus.`, "success");
      }
      sessionStorage.removeItem("refCode");
    } catch (e: any) {
