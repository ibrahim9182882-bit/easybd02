...
// Comprehensive Telegram startapp / referral parameter extractor
function extractTelegramStartParam(): string | null {
  try {
    // 1. Direct WebApp initDataUnsafe.start_param
    if (window.Telegram?.WebApp?.initDataUnsafe?.start_param) {
      const p = String(window.Telegram.WebApp.initDataUnsafe.start_param).trim();
      if (p) return p;
    }

    // 2. Parse from WebApp initData query string
    if (window.Telegram?.WebApp?.initData) {
      const parsed = new URLSearchParams(window.Telegram.WebApp.initData);
      const p = parsed.get("start_param") || parsed.get("startapp") || parsed.get("start") || parsed.get("ref");
      if (p && p.trim()) return p.trim();
    }

    // 3. Parse from URL Search Query (?tgWebAppStartParam=... or ?startapp=... or ?start=... or ?start_param=...)
    if (window.location.search) {
      ...
      const searchParam =
        ...
        urlParams.get("start") ||
        urlParams.get("ref");
      if (searchParam && searchParam.trim()) return searchParam.trim();

      // Check nested tgWebAppData in search query
      const rawDataInQuery = urlParams.get("tgWebAppData");
      if (rawDataInQuery) {
        const parsedData = new URLSearchParams(rawDataInQuery);
        const p = parsedData.get("start_param") || parsedData.get("startapp") || parsedData.get("start") || parsedData.get("ref");
        if (p && p.trim()) return p.trim();
      }
    }
    ...
    if (window.location.hash) {
      ...
      const hashDirect =
        ...
        hashParams.get("ref");
      if (hashDirect && hashDirect.trim()) return hashDirect.trim();

      const rawDataInHash = hashParams.get("tgWebAppData");
      if (rawDataInHash) {
        const parsedData = new URLSearchParams(rawDataInHash);
        const p = parsedData.get("start_param") || parsedData.get("startapp") || parsedData.get("start") || parsedData.get("ref");
        if (p && p.trim()) return p.trim();
      }
    }
  } catch (err) {
    console.warn("Failed to extract start_param:", err);
  }
  return null;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Comprehensive startapp / referral parameter extraction
    const detectedStartParam = extractTelegramStartParam();
    if (detectedStartParam) {
      const cleanRef = detectedStartParam.trim().toUpperCase();
      sessionStorage.setItem("pendingRefCode", cleanRef);
      localStorage.setItem("pendingRefCode", cleanRef);
      sessionStorage.setItem("refCode", cleanRef);
      localStorage.setItem("refCode", cleanRef);
      setRegRefCode(cleanRef);
    } else {
      const existingRef =
        sessionStorage.getItem("pendingRefCode") ||
  ...
  const initUserProfile = async (uid: string, tgUser: any, customName?: string) => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        // NEW USER REGISTRATION: Create initial profile
        ...
        const initialData: UserProfile = {
          uid,
          telegramId: tgUser ? String(tgUser.id) : "web",
        ...
        const storedRef =
          ...
          localStorage.getItem("refCode");
        if (storedRef) {
          await processReferral(storedRef, uid, true /* isNewUserRegistration */);
        }
      } else {
        // EXISTING USER
        ...
        // If existing user already exists before opening referral link, do not process automatic referral rewards
  ...
  const processReferral = async (rawCode: string, currentUid: string, isNewUserRegistration: boolean = false) => {
    if (!rawCode || !currentUid) return;
    ...
    try {
      // 1. Resolve Referrer User ID across registries
      ...
      // Secondary: public users collection lookup by referralCode
      if (!referrerId) {
        for (const c of codesToTry) {
          try {
            const usersQuery = query(collection(db, `${artifactRoot}/public/data/users`), where("referralCode", "==", c));
            const uSnap = await getDocs(usersQuery);
            if (!uSnap.empty) {
              referrerId = uSnap.docs[0].id;
              matchedCode = c;
      ...
      const signupReward = Number(settings.signupBonus) || 100;
      const referReward = Number(settings.referralBonus) || 500;
      ...
      let rewardSuccessfullyProcessed = false;

      // ATOMIC TRANSACTION: Ensures balance, totalRefers, and referredBy are updated safely
      await runTransaction(db, async (transaction) => {
        const mySnap = await transaction.get(currentUserRef);
        if (!mySnap.exists()) return;
        const myData = mySnap.data() as UserProfile;

        // Prevent Duplicate / Re-Referrals
        ...
        const myNewBal = Number(myData.balance || 0) + signupReward;
        const myNewTotal = Number(myData.totalEarned || 0) + signupReward;

        const refNewBal = Number(refData.balance || 0) + referReward;
        const refNewTotal = Number(refData.totalEarned || 0) + referReward;
        const refNewCount = Number(refData.totalRefers || 0) + 1;
        ...
        transaction.set(myEarningsRef, {
          amount: signupReward,
          source: `Signup Welcome Bonus (${matchedCode})`,
          timestamp: serverTimestamp(),
        });
        ...
        rewardSuccessfullyProcessed = true;
      });

      if (updatedMyData && rewardSuccessfullyProcessed) {
        setUserData(updatedMyData);
        // Sync public mirror for current user
        await setDoc(doc(db, `${artifactRoot}/public/data/users`, currentUid), updatedMyData, { merge: true });

        // Sync public mirror for referrer
        try {
          const refPubSnap = await getDoc(doc(db, `${artifactRoot}/public/data/users`, referrerId));
          if (refPubSnap.exists()) {
            const rpd = refPubSnap.data();
            await setDoc(
              doc(db, `${artifactRoot}/public/data/users`, referrerId),
              {
                balance: Number(rpd.balance || 0) + referReward,
                totalEarned: Number(rpd.totalEarned || 0) + referReward,
                totalRefers: Number(rpd.totalRefers || 0) + 1,
              },
              { merge: true }
            );
          }
        } catch (e) {
          console.warn("Public user mirror sync failed:", e);
        }

        showToast(`Referral applied! Received +${signupReward} Welcome Bonus 🎉`, "success");
      }
  ...
  const shareOnTelegram = () => {
    ...
    const botLink = `https://telegram.me/${botName}?startapp=${userData.referralCode}`;
    const text = `Join EasyEarnBD & get ${settings.signupBonus} Coins Bonus! 🚀\nPlay Games & Earn Real Money.\n\n👇 Click here to start earning now:`;
    const url = `https://telegram.me/share/url?url=${encodeURIComponent(botLink)}&text=${encodeURIComponent(text)}`;
